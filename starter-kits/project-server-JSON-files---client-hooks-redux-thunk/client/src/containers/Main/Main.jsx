...
const Main = (props) => {
  ...
  const getMovies = async (updatedSearchText, isSearchChange, isLoadMoreClick) => {
    onLoadMoviesStart(true);
    let intPageNumber = parseInt(pageNumber);
    let updatedPageNumber = isSearchChange ? 1 : intPageNumber > 0 ? intPageNumber : 1;
    if (isLoadMoreClick) {
      updatedPageNumber++;
    }
  ...
  useEffect(() => {
    const initiateMovies = async () => {
      await getMovies(null, false, false);
    };
    initiateMovies();
  }, []);

  const handleLoadMoreButtonClick = async () => {
    await getMovies(null, false, true);
  };
